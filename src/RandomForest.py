...
class RandomForest:
    ...
    def fit(self, X, y):
        ...
        self.trees = []
        n_samples, n_features = X.shape
        n_features_to_consider = int(np.sqrt(n_features)) if self.max_features == 'sqrt' else self.max_features
    ...
    def predict(self, X):
        ...
        # 投票决定最终预测
        majority_vote = [Counter(col).most_common(1)[0][0] for col in tree_preds.T]
